...
class Gangster {
  given(downstreams) {
    this.spies = downstreams;
    this._givenCalled = true;
    return this;
  }
  ...
  response(status, body, headers) {
    return this._setExpectedResponse(status, body, headers, false);
  }

  exactResponseBody(status, body, headers) {
    return this._setExpectedResponse(status, body, headers, true);
  }

  expectResponse(status, body, headers) {
    return this._setExpectedResponse(status, body, headers, false);
  }

  expectExactResponse(status, body, headers) {
    return this._setExpectedResponse(status, body, headers, true);
  }

  _setExpectedResponse(status, body, headers, matchExactResponseBody) {
  ...
  async run() {
    if (!this._givenCalled) {
      throw new Error(
        'given() must be called before run(). Use given([]) when there are no downstream dependencies.'
      );
    }

    const spies = this.spies || [];
    this.spies = undefined;
    this._givenCalled = false;

    // Initialize the providers
    await Promise.all(spies.map((provider) => provider.init()));
    ...
    const resolvedProvidersWithSpies = await Promise.all(
      spies.map((provider) => provider.spy())
    );
